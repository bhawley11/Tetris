...
TETRIS.screens['ai'] = (function() {
    ...
    function gameLoop(time) {
        ...
        if(!bestMoveFound){
            var newBoard = TETRIS.objects.GameBoard(),
                newShape = TETRIS.objects.Shape();

            newBoard.cloneEverything(gameBoard);
            newShape.createShape(currentShape.getShapeAbbrev());
            if(newShape.checkSpawnLocation(newBoard)){
                newShape.spawn(newBoard);
                bestMove = TETRIS.AI.findBestMove(newBoard, newShape);
                bestMoveFound = true;
            }
        }
    ...
    function update() {
        ...
        if(gameOver){
            ...
            if(bestMove.rotation === currentShape.getRotateState()){
                var temp = currentShape.getLowerLeftLocation();
                if(bestMove.location.x < temp.x){
                    currentShape.moveLeft(gameBoard);
                }
                else if(bestMove.location.x > temp.x){
